...
@Component
@Aspect
public class NetResourceBookAspect {
	private static Logger logger=LoggerFactory.getLogger(NetResourceBookAspect.class);
	@Resource
	 ...
	 * 指定切入点 
	 */
	@Pointcut("execution(* com.wonders.frame.iims.dao.NetResourceBookDao.save(..))")
	...
	public void addResourceLog(JoinPoint joinPoint, Object rtv) throws Throwable{
		...
		NetResource resource=(NetResource)genericService.executeMethod("netResource",
				"findFinishApplyBySwitchDeviceCodeAndPort", new Class[] {String.class,String.class}, new Object[] {switchDeviceCode,port});
		...
		String vlan=(String)ReflectUtil.invokeGet(obj, "vlan");
		String netStatus=(String)ReflectUtil.invokeGet(obj, "netStatus");
		String interfaceInfo=(String)ReflectUtil.invokeGet(obj, "interfaceInfo");
		String ip=(String)ReflectUtil.invokeGet(obj, "ip");
		String mac=(String)ReflectUtil.invokeGet(obj, "mac");
		String gateway=(String)ReflectUtil.invokeGet(obj, "gateway");
		...
		else if(!isValueEquals(resource.getVlan(),vlan)){
			hasChange=true;
		}
		else if(!isValueEquals(resource.getNetStatus(),netStatus)){
			hasChange=true;
		}
		else if(!isValueEquals(resource.getInterfaceInfo(),interfaceInfo)){
			hasChange=true;
		}
		else if(!isValueEquals(resource.getIp(),ip)){
			hasChange=true;
		}
		else if(!isValueEquals(resource.getMac(),mac)){
			hasChange=true;
		}
		else if(!isValueEquals(resource.getGateway(),gateway)){
			hasChange=true;
		}
		...
		if(hasChange){
			resource=(NetResource)genericService.executeMethod("netResource",
					"updateAllDataById", 
					new Class[] {String.class,String.class,String.class,String.class,Date.class,
							...
							new Object[] {userName,contactPhone,usePlace,applyBusinessType,applyUseTime,
					applyReason,equipmentType,implementEngineer,implementUser,implementTime,
					implementPlace,applyDept,applyDeptId,applyType,post,
					vlan,netStatus,interfaceInfo,ip,mac,
					gateway,sysName,applyer,applyerLoginname,applyTime,
					remarks,id});
		
			logger.debug("update NetResource success:id="+resource.getId()+" \n");
		}
