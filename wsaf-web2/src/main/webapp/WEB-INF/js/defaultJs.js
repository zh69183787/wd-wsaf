...
$.getid = function(modname){
	...
	{
		var _mid=$(count[i]).parent().next().html();
		if(_mid!=""&&_mid!=null)
		{
			_Id=_Id+_mid+",";	
		}
				...
				$(modname +" table>tbody tr").click(function()
				   {
					    ...
					    var _othermod="",this_id=$(this).children("td").eq(1).text(),urlopen=menuUrl.split("/");
...
//保存子节点
$.saveBind = function(url,pmodname,nmodname){
   ...
   {
	$.getJSON(url,{ pType: _pType, pId: _pId, nType: _nType,nId: _nId }, function(data, status, xhr) {
		if (status == "success" && data != null)
		{
