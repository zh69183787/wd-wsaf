...
<%! 
    ...
    private String toHex(byte[] buffer){  
     ...
     private void saveCookie(HttpServletResponse response,String Name,String value) throws UnsupportedEncodingException{
    		Cookie cookie = new Cookie("iims_"+Name, java.net.URLEncoder.encode(value,"utf-8"));
			cookie.setPath("/");		
...
<html>
  ...
  if(action!=null&&action.equalsIgnoreCase("logout")){
  		Cookie cookie = new Cookie("iims_token", null);  
        cookie.setMaxAge(0);  
     	cookie.setPath("/");	
        response.addCookie(cookie);	 
  	...
  	if(returnUrl!=null){
		...
		try {
	       ...
	       /*具体cookie的存储模式需要客户端自行处理*/
						...
						request.getSession().setAttribute("userId",eUserId.getTextTrim());
						request.getSession().setAttribute("loginName",eLoginName.getTextTrim());		
						request.getSession().setAttribute("userName",eUserName.getTextTrim());	
						request.getSession().setAttribute("deptId",eDeptId.getTextTrim());	
						request.getSession().setAttribute("deptName",eDeptName.getTextTrim());	
						request.getSession().setAttribute("token",eToken.getTextTrim());	
