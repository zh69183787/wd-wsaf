   ...
   <script src='js/dynatree/jquery.dynatree.min.js' type="text/javascript"></script>
    ...
    <script type="text/javascript">
        $(function () {
        	...
        	//$.getUserNameInfo("${pageContext.request.contextPath}", "?deptId=" + $("#company_Id").text());
       
        	var _form=$("form");
        	for(var i=0;i<_form.length;i++)
        	{
        		var otherData="role="+$(_form[i]).children("table").attr("id")+"&companyId="+$("#company_Id").text();
        	...
        	$(".addManager").click(function () {
        		...
        		var _name=$(this).parents("table").attr("id"),otherData="&companyId="+$("#company_Id").text()+"&company="+$("#mycompany").val()+"&role="+_name;
        		if(_name=="member")
        			{
        	...
        	$("#btnSubmit").click(function () { 
            	...
            	{
            		var _name=$(_form[j]).children("table").attr("id"),otherData="&companyId="+$("#company_Id").text()+"&company="+$("#mycompany").val()+"&role="+_name;
            		if(_name=="member")
            			{
        	...
        	var treeLoad=false;//标示树形结构是否构建完毕
              ...
              type: 'post',
                  ...
                  success: function (data) {
                         ...
                         $(".selPersonTable").dynatree({
                               ...
                               selectMode: 3,
                               checkbox: true,
                               children:{title:"当前单位",key:$("#company_Id").text(),isFolder:true,isLazy: true}             
                          });
             ...
             $(".submit2").click(function () {
            		if(treeLoad2){        
            			   var _key="",_title="",selectArr=[],selectTitleArr=[],tree=$(".selPersonTable").dynatree("getTree"), root=tree.getNodeByKey($("#company_Id").text());
             ...
             $(".submit").click(function () {
          		if(treeLoad){   
          			//以下是获取被选中节点并组合返回信息的逻辑                 
          			   var selectArr=[],selectTitleArr=[],tree=$(".selPersonTable").dynatree("getTree"), root=tree.getNodeByKey($("#company_Id").text());
