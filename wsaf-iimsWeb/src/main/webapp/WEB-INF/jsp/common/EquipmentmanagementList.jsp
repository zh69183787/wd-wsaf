...
var SelectObj="",IronMan=false,operation;
var Iframenow=0;
	$(document).ready(function() {
		...
		$("#cz").click(function(){
		$("#iframe_xz tbody input,#iframe_xz tbody select,#iframe_xz tbody textarea").val("");
...
operation="save";
  ...
  checkequipmentNum($("#ifrom2").serialize());
   if(Mach!=1&&checkNum!=2){
   $.ajax({
					type : 'POST',
...
$.setBtn = function(){
   ...
   function checkequipmentNum(str){
    checkop=1;
    var strs= str.split("&"); var mach="";
	            $.each(strs,function(index,infs){
                   ...
                   //totalRecord
					type : 'POST',
					url : "${pageContext.request.contextPath}/basicCrud/equipment/page?equipmentNum="+inf[1],
					data :"",
					dataType : 'json',
					cache : false,
					error : function() {
					...
					success : function(num) {
					   if(num.success==true){
				         ...
				         if(num.result.pageInfo.totalRecord!=0){
					          Mach=1;
					          checkNum=2;
					          alert("编号已存在！");
...
<body>
    ...
    <div id="iframe_xz" class="f_window"
		...
		<div class="button t_c">
			...
			<input type="button" id="cz" onclick="" value="重置" />
		</div>
    </div>
	<div class="main">
		<!--Ctrl-->
		...
		<div class="pt45">
			...
			<div class="tabs_2 nwarp">
				<ul class="nwarp">
					 <li class="selected"><a href="${pageContext.request.contextPath}/equipment/EquipmentList"><span>设备管理台账</span> </a>
					</li>
