...
<html>
  <head>
    ...
    <script type="text/javascript">
												...
												if($("input[name=btnISPass]:checked").val()=="pass")
										         {
													...
													var d = new Date();
														...
														if(entry.status=="innerApproval")
														  {		
														 otherData+="&id="+id+$.getPerson("${pageContext.request.contextPath}")+"&handler=方政&handlerLoginname=G01007000383&status=infoDeptApproval";
														  ...
														  {		
														   ...
														   switch(cbReview.val())
															...
															if(cbReview.val()=="serviceDeptApproval"){
																otherData+="&id="+id+"&initiator=方政&initiatorLoginname=G010070003832549&handler=&handlerLoginname=&status=serviceDeptApproval";
																otherData+="&suggestion="+$("[name=suggestion]").val()+"&initiateTime="+d.getTime();
															}else{
																otherData+="&id="+id+"&initiator=方政&initiatorLoginname=G010070003832549&handler=方政&handlerLoginname=G010070003832549&status=selfDealWith";
																otherData+="&suggestion="+$("[name=suggestion]").val()+"&initiateTime="+d.getTime();
															}
														}
														else if(entry.status=="serviceDeptApproval")
														  {		
														 otherData+="&id="+id+$.getPerson("${pageContext.request.contextPath}")+"&handler=&handlerLoginname=&status=waitForImplement";
														 otherData+="&suggestion="+$("[name=suggestion]").val()+"&initiateTime="+d.getTime();
														}
														else if(entry.status=="selfDealWith")
														{
															
														 otherData+="&id="+id+"&initiator=方政&initiatorLoginname=G010070003832549&handler=&handlerLoginname=&status=finish";
														 otherData+="&suggestion="+$("[name=suggestion]").val()+"&initiateTime="+d.getTime();
														}
														
														else if(entry.status=="waitForImplement"){
														 otherData+="&id="+id+$.getPerson("${pageContext.request.contextPath}")+"&handler=&handlerLoginname=&status=finish";
														 otherData+="&suggestion="+$("[name=suggestion]").val()+"&initiateTime="+d.getTime();
														}
													  ...
													  if(isleader=="本部门领导")
														{
															...
															otherData+=data+"&handler=胡波&handlerLoginname=G002000000332549&status=innerTurnBack";
														}else if(isleader=="信息管理中心方政"){
															...
															otherData+=$.getPerson("${pageContext.request.contextPath}")+"&handler=胡波&handlerLoginname=G002000000332549&status=serviceDeptBack";
														}
								                	   ...
								                	   if(id!=null)
								                		   ...
								                		   otherData+="&id="+id+"&suggestion="+$("[name=suggestion]").val()+"&initiateTime="+d.getTime();
								                		   	...
								                		   	if($("[name=suggestion]").val().length>50||$("[name=suggestion]").val().length<5){
															alert("输入意见超过50个字");
															return false;
														}
														$(this).attr("disabled", true);
		...
		function add(firUrl){
			window.open("/iims/accountResource/systemAccountAdd");
		}
        ...
        <div class="pt45">
                    ...
                    <div class="tabs_2">
                        <ul>
                            ...
                            <li class="selected"><a><span>系统账户申请</span></a></li>
        	...
        	<table width="100%"  class="table_1">
                <tbody>
                   <tr class="tit">
                       ...
                       <td class="sort"><a ><span class="fl">申请业务类型</span></a></td>
