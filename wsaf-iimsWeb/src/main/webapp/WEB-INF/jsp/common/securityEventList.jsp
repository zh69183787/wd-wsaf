        ...
        $(document).ready(function () {
            $.getListDetails = function (firUrl) {
                ...
                if (firUrl != "") {
                    $.ajax({
                        async: false,
                        url: firUrl + "/basicCrud/manager/all?companyId=" + $("[name=reportUnitId]").val(),
                        type: 'post',
                                ...
                                $.each(data.result, function (entryIndex, entry) {
                                	  if (entry.loginName == $("#myname").text())
                                    {
                                         if (entry.role == "producePrincipal") 
                                          ...
                                          else if (entry.role == "enterprisePrincipal") 
                                          {	  
                                       	   _retrunstr = 3;
                                          }
                                	 }
            ...
            $.securityEventSearchForm = function (firUrl, paging, isleader) {
                if (firUrl != "") {
                	//$("[name=eventCate]").val("1");
                    var otherData = "&companyId=" + $("[name=reportUnitId]").val();

                    if (isleader==1||isleader==3) {
                        $("#addbtn").remove();
                           otherData += "&status_in=businessReview,deptLeaderReview,infoDeptReview,finish,maintainReview";
                    } 
                    else if(isleader==2) {
                        $("#addbtn").remove();
                        otherData += "&status_in=deptLeaderReview,infoDeptReview,finish,maintainReview";
                    } 
                    else {
                    ...
                    if (data.success == true ) {
                        ...
                        $.each(data.result.content, function (entryIndex, entry) {
                            ...
                            if (isleader==1||isleader==3) {
                                ...
                                if (entry.status == "businessReview") {
                            	 ...
                            	 if (entry.status == "deptLeaderReview") {
                                 	...
                                 	if(entry.handlerLoginname != $("#myname").text())
                                     {
                                 		...
                                 		{
                                     newpage = "securityEventReviewDepSafety";
                                     strBtn="审查";
                                 		}
                                 }
                                 else {
                                ...
                                if (entry.status == "draft" || entry.status == "businessBack" || entry.status == "deptLeaderBack" || entry.status == "infoDeptBack") {
                                    newpage = "securityEventEdit";
                                    strBtn="修改";
                                }
                                else {
                                    newpage = "securityEventDetail";
        ...
        <div class="pt45">
            ...
            <div class="filter">
                        ...
                        <form id="projectAppform">
                            <table class="nwarp" width="100%" border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td>
                                      <select name="eventCate"  class="input_large">
                                        </select>
                                    </td>
                ...
                <div class="fn clearfix">
                    ...
                    <input type="button" id="addbtn" value="新 增" onclick="window.open('${pageContext.request.contextPath}/securityEvent/securityEventAdd');" class="fr">
                </div>
            </div>
            <!--Filter End-->
            <!--Table-->
