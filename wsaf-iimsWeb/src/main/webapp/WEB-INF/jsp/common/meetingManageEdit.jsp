...
<script type="text/javascript">
	$(function() {
		 $("[name=startDate]").datepicker({onSelect: function(dateText, inst) {$(".ui-state-default").removeAttr("href"); },dateFormat:"yy-mm-dd"});
		var issue_num=0;
		QueryString.Initial();
...
$.FileDel = function(firUrl,_id,isRead){
							...
							{	
								if (data.success == true) 
								{
									...
									for ( var item in entry) 
									{
									     if(isRead)
										{
									    	 info+="<tr name=\""+entry[item].id+"\"><td>"+entry[item].fileName+"</td><td>"+entry[item].fileSize+"</td><td>"+entry[item].uploadDate+"</td><td><a href=\"${pageContext.request.contextPath}/attach/api/download/"+entry[item].id+"\">下载</a></td><td><a class=\"del\">删除</a></td></tr>";
										}
									     else
									    	 {
								...
								if (status == "success" && data != null)
								{
									if (data.success == true) 
									{
										var entry=data.result,isreader=false,otherdata="modelName=meeting&modelId="+id,otherdata2="";		
										  ...
										  var info=$.getFileInfo(firUrl,otherdata2,true);
										...
										{ 	
											  if(item=="topic")
												...
												{
													  var dataObj2=jQuery.parseJSON(dataObj[sitem]),_obj="";
													  ...
													  if(sitem!="meetingSummary")
												    {
														   ...
														   var info=$.getFileInfo(firUrl,otherdata2,true);
														   issue_num+=1;
														    $("#addIssue").parents("tr").after("<tr class=\"yiti\"><td class=\"lableTd t_r\">会议资料</td><td colspan=\"3\"><table width=\"100%\" class=\"table_issue"+issue_num+"\"><thead><tr><td colspan=\"4\" >议题：  <input  type=\"text\" value=\""+sitem+"\" style=\"width:360px\" class=\"txtissue\"/></td><td style=\"color:Blue;cursor:pointer;text-align:right\" class=\"delIssue\">删除</td></tr><tr><td  colspan=\"5\"><input id=\"upload_photo"+issue_num+"\" class=\"upload_button\" type=\"button\" value=\"浏览...\" /><div style=\"display:none\" class=\"myissue\">issue"+issue_num+"</div><input type=\"button\" value=\"上传\"  onclick=\"$('#upload_photo"+issue_num+"').uploadify('upload','*');\"><input type=\"button\" value=\"取消\"  onclick=\"$('#upload_photo"+issue_num+"').uploadify('cancel','*');\"></td></tr><tr><td>文件名</td> <td>大小(kb)</td><td>上传时间</td><td colspan=\"2\">操作</td></tr></thead><tbody>"+info+"</tbody><tfoot><tr><td colspan=\"5\" style=\"text-align:right;cursor:pointer;color:Blue;border-bottom:0\" class=\"setPermission\" >设置权限</td></tr></tfoot></table></td></tr>");
													...
													var safety=dataObj2.share.split(",");
													for (var x=0;x<safety.length ;x++ ) 
													{ 
														var _safety=$(_obj+" .selPersonTable tbody tr");
														for (var y=0;y<_safety.length ;y++ ) 
														{ 
															if($(_safety[y]).find(".LoginName:first").text()==safety[x])
																$(_safety[y]).find("input[name=cbPerson]").attr("checked","checked");
															
														}
													} 
													
													
													$(".yiti table tbody a.del").click(function(){
															$.FileDel("${pageContext.request.contextPath}",$(this).parents("tr").attr("name"),true);
															  $(this).parents("tr:first").remove();
													 });
											  ...
											  {
												  $("[name=startDate]").val(entry[item].trim());
											  }
		...
		$("#btnSubmit").click(function(){
			...
			var otherData="&id="+QueryString.GetValue('id')+"&topic={",_obj=$(".winPermission");
		...
		if(QueryString.GetValue('id')!=null)
		{
			$("#btndel2").click(function(){
				...
				$.postsave2("${pageContext.request.contextPath}","projectPlanForm","meeting","&removed=1&id="+QueryString.GetValue('id'),"/meeting/meetingManageList",false,true,false,"");
