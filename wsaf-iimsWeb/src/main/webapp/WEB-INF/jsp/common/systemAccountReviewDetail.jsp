...
<html>
  <head>
	...
	<script src="js/systemAccount.js"></script>
								...
								if (data.success == true&&data.result&&JSON.stringify(data.result) != "[]") 
								{
									...
									for ( var item in entry) 
									{
									 ...
									 $("#tbfj tbody a.del").click(function(){
											...
											$.getFileInfo("${pageContext.request.contextPath}","modelName=accountResource&modelId="+QueryString.GetValue('id'),true);
									});
								}
							}
						}
		...
		var isleader= $.isleader("${pageContext.request.contextPath}");
			...
			$.postResourceManageDetail = function(firUrl,id,isread,isPreliminaryPersonnel){
			 ...
			 if(firUrl!="")
			   {	
					 ...
					 $.ajax({
						   async: false,
						    url : firUrl+"/basicCrud/accountResource/get/"+id,
							type : 'post',
							success: function(data, status, xhr) {
								if (status == "success" && data != null)
								{
										...
										var entry=data.result;
										 if(entry.status=="waitForImplement"){
									          ...
									          if(entry.status=="waitForImplement"){
												...
												window.close();
												location.href = Url
														+ "/accountResource/systemAccountList";
						...
						if(entry.status=="waitForImplement"||entry.status=="selfDealWith"){
                    			...
                    			item+="<td> <input  type='text' class='input_large' name='implementEngineer' /></td>";
                 				...
                 				item+="<tr>";
                				item+="<td class='lableTd t_r'><div class='red'> *</div>账户名称</td>";
                    			...
                    			item+="<td> <input  type='text' class='input_large' name='implementUser' /></td>";
                 				...
                 				item+="<td> <input  type='text' class='input_large' name='place' /></td>";
            				    item+="</tr>";
							...
							if(entry.status=="finish"){
                    			...
                    			item+="<td> <input disabled type='text' class='input_large' name='implementUser' /></td>";
               				    item+="</tr>";
               				 	...
               				 	item+="<tr>";
          						item+="<td class='lableTd t_r'><div class='red'> *</div>开通位置</td>";
              					item+="<td> <input disabled type='text' class='input_large' name='place' /></td>";
         				    	item+="</tr>";
		  ...
		  $("#closebtn1").click(function(){
			...
			$("[name=applyBusinessType]").change(function(){
					...
					if($("[name=applyBusinessType]").val()!="other"){
					    $("#sys").css("display","none");
					} else{
						$("#sys").css("display","table-row");
					}       	
	           }); 
	           if($("[name=applyBusinessType]").val()!="other"){
					    $("#sys").css("display","none");
                ...
                <div class="divB">
                	...
                	<div class="con">
						...
						<ul class="button clearfix" style="filter:Gray">
                        	...
                        	<li class="disable"><a  class="bat" href="/iims/accountResource/systemAccountList">批量审核</a></li>
            	...
            	<div class="w_bg">
                	...
                	<div class="Bottom">
                    	...
                    	<div class="Top">
                        	 ...
                        	 <h1 class="t_c">上海申通地铁集团有限公司<br>系统账户申请/注销审批办理表</h1>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_2">
                ...
                <tr>
                  ...
                  <td> 
                      ...
                      <option value="" selected>请选择</option>
                     <option value="emailAccount" >企业邮箱账户</option>
                      <option value="portalAccount" >协同平台账户</option>
               ...
               <tr>
                  ...
                  <td ><input disabled type="text" class="input_large" name="dept" /></td>
                   <td class="lableTd t_r"><div class="red"  id="red" > *</div>职务</td>
                  <td >                 	
                  <select disabled name="post" class="input_large" name="post" >
                      <option value="" selected>请选择</option>
                      <option value="principal" >部门正职（主持工作）</option>
                      <option value="deputy" >部门副职</option>
               ...
               <tr>
                  ...
                  <td class="lableTd t_r"><div class="red"> </div>员工工号</td>
                  <td ><input  disabled type="text" class="input_large" name="loginName" /></td> 
                      
                </tr>
                 <tr style="display:table-row" id="sys">
                  <td class="lableTd t_r"><div class="red"> *</div>系统名称</td>
                  <td ><input disabled type="text" class="input_large" name="sysName" /></td> 
                </tr>
                <tr>
