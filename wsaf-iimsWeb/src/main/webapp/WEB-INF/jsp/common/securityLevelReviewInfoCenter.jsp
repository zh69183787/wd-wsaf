        ...
        $(function () {
                        ...
                        success: function (data, status, xhr) {
                            ...
                            if (status == "success" && data != null) {
                                if (data.success == true ) {
                                    var entry = data.result, _lv = entry.sysSecurityLevel.replace("level", ""), suggestLv = $("[name=suggestLv]");
                                    for (var i = 0; i < parseInt(_lv) - 1; i++) {
                                        $(suggestLv[i]).attr("disabled", "disabled");
                                    }

                                    $("#btnSubmit2").click(function () {
                                        if ($("input[name=btnISPass]:checked").val() == "pass") {
                                            var otherData = "", yanzhen = "", _mlv = "", lv_china = "";

                                            if ($("[name=cbReview]:checked").val() == null || $("[name=cbReview]:checked").val() == "")
                                                yanzhen += "请选择一个审核领导\r\n";
                                            ...
                                            otherData += "&suggestion="+$("#mySuggest").val();

                                            if (id != null) {
                                                var d = new Date();
                                                var str_china = "";
                                                switch ($("[name=cbReview]:checked").val()) {
                                                    case "G020001000492549":
                                                        str_china = "金涛";
                                                        break;
                                                    case "G001000002232549":
                                                        str_china = "黄天印";
                                                        break;
                                                }

                                                otherData += "&handler=" + str_china;
                                                otherData += "&handlerLoginname=" + $("[name=cbReview]:checked").val();
                                                otherData += "&initiator=" + entry.handler;
                                                otherData += "&initiatorLoginname=" + entry.handlerLoginname;

                                                otherData += "&id=" + id + "&status=infoDeptLeaderReview" + "&initiateTime=" + d.getTime();
                                            }
                                            ...
                                            var otherData = "", _msg = "", obj = $(".reviewNotPass input[type=checkbox]:checked");
                                            var d = new Date();

                                            for (var i = 0; i < obj.length; i++) {
                                            	
                                            	if($(obj[i]).attr("id")!="mynotifyDeptLeader")
                                                _msg += $(obj[i]).parents(".reviewNotPass").find(".msg div").text().trim() + ":" + $(obj[i]).parents(".reviewNotPass").find(".msg textarea").val() + ",";
                                            }
                                            _msg = _msg.substring(0, _msg.length - 1);

                                            if (id != null) {
                                                otherData += "&id=" + id + "&status=infoDeptBack";
                                            }

                                            if ($("#mynotifyDeptLeader").attr("checked") == "checked")
                                                otherData += "&notifyDeptLeader=YES";

                                            otherData += "&suggestion=反馈意见," + _msg + "&handler=" + entry.applyer;
                                            otherData += "&handlerLoginname=" + entry.applyerLoginname;
                                            otherData += "&initiator=" + entry.handler;
                                            otherData += "&initiatorLoginname=" + entry.handlerLoginname;
                                            $(this).attr("disabled", true);
                                            $.postsave2(firUrl, "projectPlanForm", "securityLevel", otherData + "&initiateTime=" + d.getTime(), "/securityLevel/securityLevelInfoCenterList", false, true, false, "");
                                        }
            ...
            if (QueryString.GetValue('id') != null) {
                ...
                $("#btnEdit").click(function () {
                    location.href = "${pageContext.request.contextPath}/securityLevel/securityLevelLeaderEdit?id=" + QueryString.GetValue('id') + "&toUrl=securityLevelReviewInfoCenter";
                });
                ...
                $("[name=suggestLv]").click(function(){
                	 if ($("#cbSuggest").attr("checked") != "checked")
                		 return false;
                	 
                	if ($("[name=suggestLv]:checked").length > 0) {
                        _mlv = $("[name=suggestLv]:checked").val().replace("level", "");

                    }
                    else {
                        _mlv = $("[name=sysSecurityLevel]").val().replace("level", "");
                    }
                    ...
                    $("#mySuggest").val("拟同意该系统备案（" + $("[name=sysSecurityLevel] option:selected").text().substring(1, $("[name=sysSecurityLevel] option:selected").text().length) + "）,建议系统主管单位按（" + lv_china + "级）等保标准执行 。");
                });
                
                $("#cbSuggest").click(function(){
                   ...
                   $("#mySuggest").val("拟同意该系统备案（" + $("[name=sysSecurityLevel] option:selected").text().substring(1, $("[name=sysSecurityLevel] option:selected").text().length) + "）,建议系统主管单位按（" + lv_china + "级）等保标准执行 。");
               		 ...
               		 {
               	    $("#mySuggest").val("拟同意该系统备案（" + $("[name=sysSecurityLevel] option:selected").text().substring(1, $("[name=sysSecurityLevel] option:selected").text().length) + "）");
               		 }
               		});
                ...
                $("[name=btnISPass]").click(function () {
                    ...
                    switch ($("input[name=btnISPass]:checked").val()) {
                        case "pass":
                            ...
                            $("#btnSubmit2").val("提交审查");
                            $(".reviewPass").css("display", "table-row");
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr id="reviewinfo">
                    ...
                    <td>
                        ...
                        审查意见：<input name="btnISPass" type="radio" value="pass" style="margin-left: 20px">通过<input name="btnISPass" type="radio" value="notPass" style="margin-left: 10px">不通过</td>

                </tr>
                <tr class="reviewPass" style="display: none">
                    <td class="td_1">
                        <input type="checkbox" id="cbSuggest"></td>
                    <td>
                        
                        建议参照标准：<input name="suggestLv" type="radio" value="level1" style="margin-left: 20px">一级<input name="suggestLv" type="radio" value="level2" style="margin-left: 10px">二级
                <input name="suggestLv" type="radio" value="level3" style="margin-left: 10px">三级<input name="suggestLv" type="radio" value="level4" style="margin-left: 10px">四级<input name="suggestLv" type="radio" value="level5" style="margin-left: 10px">五级
                    </td>
                </tr>
                <tr class="reviewPass" style="display: none">
                    <td class="td_1">&nbsp;</td>
                 ...
                 <tr class="reviewPass" style="display: none">
                    <td class="td_1">&nbsp;</td>
                    <td><textarea rows="3"  id="mySuggest"></textarea> </td>
                 </tr>     
                ...
                <tr class="reviewNotPass" style="display: none">
                    <td class="td_1"><input type="checkbox" style="margin-left: 40px"></td>
                    <td class="msg">
                        
                        <div class="fl">定级不准</div> <textarea rows="3" style="width:300px;margin-left: 10px"></textarea></td>
                </tr>
                <tr style="display: none" class="reviewNotPass">
                    <td class="td_1"> <input type="checkbox" style="margin-left: 40px"></td>
                    <td class="msg">
                        <div class="fl">资料不全</div> 
                        <textarea rows="3" style="width:300px;margin-left: 10px"></textarea></td>
                </tr>
                <tr style="display: none" class="reviewNotPass">
                    <td class="td_1"><input type="checkbox" style="margin-left: 40px"></td>
                    <td class="msg">
                         <div class="fl">内容偏差</div> 
                        <textarea rows="3" style="width:300px;margin-left: 10px"></textarea></td>
                </tr>
                <tr style="display: none" class="reviewNotPass">
                    <td class="td_1"><input type="checkbox" style="margin-left: 40px"></td>
                ...
                <tr style="display: none" class="reviewPass">
                    <td colspan="2">审核领导</td>
                </tr>
                <tr style="display: none" class="reviewPass">
                    <td class="td_1">&nbsp;</td>
                    <td>
                        <input name="cbReview" type="radio" value="G020001000492549">&nbsp;金涛<input name="cbReview" type="radio" value="G001000002232549" style="margin-left: 20px">&nbsp;黄天印</td>
                </tr>
                <tr>
                ...
                <div class="w_bg">
                    ...
                    <div class="Bottom">
                        ...
                        <div class="Top">
                            ...
                            <div class="mb10 t_c">
                                <input type="button" id="btnSubmit" value="备案审查" />
                                &nbsp;
