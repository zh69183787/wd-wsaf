...
<html>
  <head>
	...
	$(function() {
		...
		$("[name=sysName],[name=implementTime],[name=implementUser]").removeAttr("disabled");
		$("[name=hostName],[name=ip],[name=software]").removeAttr("disabled");
								...
								$("#btnSubmit").val("提 交");
									$("#btnSubmit").click(function(){
										var otherData="";var d = new Date();
											otherData+="&id="+QueryString.GetValue('id')+$.getPerson(firUrl)+"&handler=&handlerLoginname=&status=finish";
											otherData+="&initiateTime="+d.getTime();
											$(this).attr("disabled", true);
											//alert(otherData);
						                	$.postsave(firUrl,"projectPlanForm",otherData);
						                	window.close();
											location.href = Url
													+ "/virtualResource/virtualResourceImplementList";
									});
										for ( var item in entry){
										  try{ 										
