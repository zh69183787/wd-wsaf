...
var Iframenow=0;
 ...
 $("#bc").click(function(){
   ...
   save();
  	...
  	$.showObjectOption = function() {
		...
		$.ajax({
					...
					success : function(obj) {
						if (obj.success) {
						    SelectObj=obj;
						   	    loadinformationToVersionAdd(obj);
						   	    checkPower();
						}
					}
				});
        ...
        $.ajax({
					...
					success : function(user) {
				     ...
				     if(user.cookies.deptId!="2549"){
				       ...
				       $("[name='dept']").find("option[value='"+user.cookies.deptId+"']").attr("selected",true);
				     }else{
				        power="yes";
				     }
	...
	}else if(QueryString.GetValue("backpage")=="1"){
	  
	    $("#fh").click(function(){
	        window.location.href="${pageContext.request.contextPath}/appCount/AppCountList";
     	});
	  }
 }
...
//operation="save";
  ...
  validate($("#ifrom_xz").serialize());
  //alert($("#ifrom_xz").serialize());
  
   if(Mach!=2){
   $.ajax({
					type : 'POST',
					url : "${pageContext.request.contextPath}/basicCrud/appVersion/save",
					data : $("#ifrom_xz").serialize(),
					dataType : 'json',
					cache : false,
					error : function() {
						alert('系统连接失败，请稍后再试1！');
	...
	<div class="main">
        ...
        <div class="tabs_2 nwarp">
        	<ul class="nwarp" >
            	...
            	<li class="selected"><a href="javascript:;"><span>正版化台账-新增</span></a></li>
            </ul>
        </div>
        <!--Tabs_2 End-->
		         <!--Filter-->
      <div class="filter">
        	<div class="query">
            	<div class="filter_search p8"><!--这里根据内容做样式调整。1、筛选：filter_sandglass 2、搜索filter_search 3、提示：.filter_tips 背景图会改变。-->
                  ...
                  <div class="mb10">
            <form id="ifrom_xz">
              <table id="table_addnew" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_2">
                <thead>
                <th colspan="4" class="f1"> <h5 class="fl">正版化台账新增</h5>
                  </th>
                    </thead>
                 <tbody>
            ...
            <div class="fn clearfix t_c">
                ...
                <input type="button" id="bc" value="保 存" />
          &nbsp;
                        <input type="button" id="cz" value="重 置" />
          &nbsp; 
                        <input type="button" id="fh" value="返 回" /></td>
            </div>
      </div>
        <!--Filter End-->
		</div>
