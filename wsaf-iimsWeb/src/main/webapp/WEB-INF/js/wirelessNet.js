...
$.setBtn2=function(firUrl){
   ...
   $("#oututBtn").click(function(){
	  var paging="";
 	  ...
 	  window.location.href=firUrl+"/basicCrud/wirelessResource/export2Excel?"+content+"&type=wirelessResource";
			...
			.click(
					function() {
						...
						window.location.href = firUrl
								+ "/basicCrud/wirelessResource/export2Excel?"
								+ content + "&type=wirelessResource";
...
$.postSearchForm = function(firUrl, paging, isleader) {
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/wirelessResource/page",
			type : 'post',
					...
					.each(
							...
							function(entryIndex, entry) {
								...
								if (entry.status == "draft"
									...
									newpage = "wirelessNetDetail";
								} else if (entry.status == "innerApproval") {
									newpage = "wirelessNetReviewDetail";
...
//	 return false;
	if (Url != "") {
		$.ajax({
			async : false,
			url : Url + "/basicCrud/wirelessResource/save",
			type : 'post',
						...
						if (status == "success" && data != null) {
							if (data.cookies
								...
								if (data.cookies.loginName == "G010070003832549") {
									_isleader = "信息管理中心方政";
								} else if (data.cookies.loginName.substring(0,12) == "G01006001622") {//G010060016222545//G080050014192925
									_isleader = "江啸栋";
								}
...
$.yanzhen2 = function() {
	...
	if ($("[name=macAddress]").val() == ""
			|| $("[name=macAddress]").val() == null) {
		_return += "\r\nMAC地址不能为空";
	}
...
$.getExport = function(firUrl, _data) {
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/wirelessResource/iimsExcel",
			type : 'Get',
			datatype : 'json',
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl
							+ "/basicCrud/wirelessResource/getEntityProperty",
					type : 'post',
					datatype : 'json',
					success : function(data, status, xhr) {
						if (data.success == true
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl
							+ "/basicCrud/workflowLog/all?ptype=wirelessResource&pid="
...
$.getSerialNo = function(firUrl){
	if (firUrl != "") {
		$.ajax({
			async : false,
			url : firUrl + "/wirelessResource/getSerialNo",
			type : 'get',
			success : function(data, status, xhr) {
				if (status == "success" && data != null) {
					var no  = "无线-"+data.serialNo
					$("[name=businessCode]").val(no);
				}
			}
		});
	}
