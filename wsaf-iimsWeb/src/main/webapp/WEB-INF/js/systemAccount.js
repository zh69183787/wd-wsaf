...
$.setBtn2=function(firUrl){
   ...
   $("#oututBtn").click(function(){
	  var paging="";
 	  ...
 	  window.location.href=firUrl+"/basicCrud/accountResource/export2Excel?"+content+"&type=accountResource";
			...
			.click(
					function() {
						...
						window.location.href = firUrl
								+ "/basicCrud/accountResource/export2Excel?"
								+ content + "&type=accountResource";
...
$.postSearchForm = function(firUrl, paging, isleader) {
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/accountResource/page",
			type : 'post',
...
$.postDele = function(Url, id) {
	...
	if (Url != "") {
		$.ajax({
			url : Url + "/basicCrud/accountResource/del/" + id,
			type : 'post',
			success : function(data, status, xhr) {
				if (status == "success" && data != null) {
...
$.postsave = function(Url, formName, otherData) {
	...
	if (Url != "") {
		$.ajax({
			async : false,
			url : Url + "/basicCrud/accountResource/save",
			type : 'post',
...
$.yanzhen = function() {
	...
	if ($("[name=applyBusinessType]").val() == "portalAccount") {
		if ($("[name=post]").val() == "" || $("[name=post]").val() == null) {
			_return += "\r\n职务不能为空";
		}
...
$.getExport = function(firUrl, _data) {
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/accountResource/iimsExcel",
			type : 'Get',
			datatype : 'json',
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl
							+ "/basicCrud/accountResource/getEntityProperty",
					type : 'post',
					datatype : 'json',
					success : function(data, status, xhr) {
						if (data.success == true
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl
							+ "/basicCrud/workflowLog/all?ptype=accountResource&pid="
