...
/*$.postSearchForm = function(firUrl,paging,isleader){
		 ...
		 if(firUrl!="")
		   {	
			 ...
			 $.ajax({
					  async: false,
					    url : firUrl+"/projectplanimposed/pageplan",
						type : 'post',
...
$.analyzeTableData= function(data, status, xhr,firUrl,isleader)
		...
		if (status == "success" && data != null)
		{
			...
			if (data.success == true) 
			{
				...
				$.each(data.result.content,function(entryIndex,entry) 
				{
					...
					tableItem+="<td><input type=\"button\" name=\"button\" id=\"edit\" value=\"详情\" onclick=\"window.open('"+firUrl+"/projectPlanTarget/"+newpage+"?id="+entry.id+"');\" \></td>";
									  ...
									  if(isleader==true&&entry.status=="waitForAudit")
										{
										  ...
										  $("#closebtn").click(function(){	
												 ...
												 if(id!=null)
							                    {
							                	  ...
							                	  $.postsave(firUrl,"projectPlanTargetForm",otherData+"&initiateTime="+d.getTime()+"&suggestion="+$("[name=suggestion]").val(),"/projectPlanTarget/projectApplicationList");
											...
											if($("input[name=btnISPass]:checked").val()=="pass")
									         {
												  var otherData=$.getOtherData();
							                	   ...
							                	   $.postsave(firUrl,"projectPlanTargetForm",otherData+"&suggestion="+$("[name=suggestion]").val(),"/projectPlanTarget/projectApplicationList");
	...
	$.postsave = function(Url,formName,otherData,toUrl){
		...
		if(Url!="")
		   {					
				 $.ajax({
					    url : Url+"/basicCrud/projectPlanTarget/save",
						type : 'post',
						  async: false,
	...
	$.getExport= function(firUrl,_data){
		...
		if(firUrl!="")
		 ...
		 $.ajax({
			    async: false,
			    url  : firUrl+"/basicCrud/projectPlanTarget/iimsExcel",
				type : 'Get',
				datatype:'json',
	...
	$.getSuggestInDept = function(firUrl,_id){
		if(firUrl!="")
		 ...
		 $.ajax({
			    async: false,
			    url : firUrl+"/basicCrud/workflowLog/all?ptype=projectPlanTarget&pid="+_id+"&sort=initiateTime desc",
				type : 'post',
				success: function(data, status, xhr) {
					if (status == "success" && data != null)
					{ 
