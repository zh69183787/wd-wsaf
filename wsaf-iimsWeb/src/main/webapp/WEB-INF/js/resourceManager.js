...
$.setBtn2=function(firUrl){
   ...
   $("#oututBtn").click(function(){
	  var paging="";
 	  ...
 	  window.location.href=firUrl+"/basicCrud/netResource/export2Excel?"+content+"&type=netResource";
			...
			.click(
					function() {
						...
						window.location.href = firUrl
								+ "/basicCrud/netResource/export2Excel?"
								+ content + "&type=netResource";
...
$.postSearchForm = function(firUrl, paging, isleader) {// isleader=="信息管理中心方政"||isleader=="维保部门"||
	if (isleader != "不是领导") {
		if (isleader == "本部门领导") {
	...
	if ($("[name=date1]").val() != "" && $("[name=date2]").val() != "") {
		...
		paging += "&applyUseTime_s=" + date1;
	} else if ($("[name=date1]").val() == "" && $("[name=date2]").val() != "") {
		paging += "&applyUseTime_e=" + date2;
	}
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/netResource/page",
			type : 'post',
...
$.analyzeTableData = function(data, status, xhr, firUrl, isleader) {
	if (status == "success" && data != null) {
		...
		if (data.success == true && data.result.content
			...
			$("tr.tableItem").remove();
			var tableItem = "", applyBusinessTypeChina = "";
			var i = 0;
			...
			$
					.each(
							...
							function(entryIndex, entry) {
								...
								tableItem = "<tr value="
										+ entry.status
										+ " class=\"tableItem\"><td class=\"t_c\"><input type=\"checkbox\"  name=\"checkbox\" value="
										+ entry.id + "></td>";
								tableItem += "<td>" + i + "</td><td>"
										+ entry.userName + "</td><td>"
										+ entry.applyDept + "</td><td>"
										+ applyBusinessTypeChina
										+ "</td><td name=\"status\" id="
										+ entry.id + " value=" + entry.status
										+ ">" + statusChina + "</td><td>";
								tableItem += entry.usePlace + "</td><td>"
										+ entry.applyUseTime + "</td>";

								var newpage = "";
								if (entry.status == "draft"
										|| entry.status == "turnBack"
										|| entry.status == "innerTurnBack") {
									newpage = "resourceManageDetail";
								} else if (entry.status == "innerApproval") {
									newpage = "resourceManageReviewDetail";
...
$.postDele = function(firUrl, id) {
	if (firUrl != "") {
		$.ajax({
			url : firUrl + "/basicCrud/netResource/del/" + id,
			type : 'post',
			success : function(data, status, xhr) {
				if (status == "success" && data != null) {
...
//	 return false;
	...
	if (Url != "") {
		$
				.ajax({
					async : false,
					url : Url + "/basicCrud/netResource/save",
					type : 'post',
					...
					success : function(data,status, xhr) {
						if (status == "success" && data != null) {
							if (data.success == true) {
								...
								window.close();
								location.href = Url
										+ "/netResource/resourceManageList";
							} else {
								alert("修改失败");
							}
						} else {
...
$.yanzhen = function() {
	...
	if ($("[name=applyBusinessType] option:selected").val() == ""
			|| $("[name=applyBusinessType] option:selected").val() == null) {
		_return += "\r\n业务申请类型不能为空";
	}
	var a = /^[0-9]*$/;
			...
			success : function(data, status, xhr) {
				if (status == "success" && data != null) {
					if (data.cookies && JSON.stringify(data.cookies) != "[]") {
						...
						// $("#lreportUnit").val(data.cookies.deptName);
						//						  
						// _retrunstr+="&reportUnit="+data.cookies.deptName;
						//						   
						//						   
						// if(data.leaders!="")
						// {
						...
						// $("#applyer").val(data.cookies.userName);
						$("[name=applyer]").val(data.cookies.userName);
						...
						// $("[name=handler]").val(data.deptLeaders);
...
$.getExport = function(firUrl, _data) {
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/netResource/iimsExcel",
			type : 'Get',
			datatype : 'json',
...
$.getMenu = function(firUrl) {
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl + "/basicCrud/netResource/getEntityProperty",
					type : 'post',
					datatype : 'json',
					success : function(data, status, xhr) {
						if (data.success == true
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl
							+ "/basicCrud/workflowLog/all?ptype=netResource&pid="
										...
										.each(
												...
												function(entryIndex, entry) {
													...
													case "信息中心领导退回":
														...
														_str = "<div class=\"node\"><b class=\"fl\">信息中心部门意见</b><div class=\"clear\"></div><div class=\"con\">";
