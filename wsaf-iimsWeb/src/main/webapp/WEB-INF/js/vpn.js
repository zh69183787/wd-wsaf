...
$.setBtn2=function(firUrl){
   ...
   $("#oututBtn").click(function(){
	  var paging="";
 	  ...
 	  window.location.href=firUrl+"/basicCrud/vpnResource/export2Excel?"+content+"&type=vpnResource";
			...
			.click(
					function() {
						...
						window.location.href = firUrl
								+ "/basicCrud/vpnResource/export2Excel?"
								+ content + "&type=vpnResource";
...
$.postSearchForm = function(firUrl, paging, isleader) {
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/vpnResource/page",
			type : 'post',
			...
			$
					.each(
							...
							function(entryIndex, entry) {
								...
								case "vpnForInnerNet":
									applyBusinessTypeChina = "集团管理网络VPN账户，访问集团内部网络";
									break;
								case "vpnForPublishSite":
									applyBusinessTypeChina = "上海地铁官方网站VPN账户，访问网站发布系统";
									break;
								case "other":
...
$.postsave = function(Url, formName, otherData) {
	...
	if (Url != "") {
		$.ajax({
			async : false,
			url : Url + "/basicCrud/vpnResource/save",
			type : 'post',
...
$.getExport = function(firUrl, _data) {
	...
	if (firUrl != "") {
		...
		$.ajax({
			async : false,
			url : firUrl + "/basicCrud/vpnResource/iimsExcel",
			type : 'Get',
			datatype : 'json',
...
$.getMenu = function(firUrl) {
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl + "/basicCrud/vpnResource/getEntityProperty",
					type : 'post',
					datatype : 'json',
					success : function(data, status, xhr) {
						if (data.success == true
	...
	if (firUrl != "") {
		$
				.ajax({
					async : false,
					url : firUrl
							+ "/basicCrud/workflowLog/all?ptype=vpnResource&pid="
