...
String configPath = Thread.currentThread().getContextClassLoader().getResource("cpqConfig.properties").getPath();
try {
...
<%! 
    ...
    private String toHex(byte[] buffer){  
     ...
     private void saveCookie(HttpServletResponse response,String Name,String value) throws UnsupportedEncodingException{
    		Cookie cookie = new Cookie("cpq_"+Name, java.net.URLEncoder.encode(value,"utf-8"));
			cookie.setPath("/");		
...
<html>
  ...
  if(action!=null&&action.equalsIgnoreCase("logout")){
  		Cookie cookie = new Cookie("cpq_token", null);  
        cookie.setMaxAge(0);  
     	cookie.setPath("/");	
        response.addCookie(cookie);	 
  	...
  	if(returnUrl!=null){
		...
		try {
	       ...
	       /*具体cookie的存储模式需要客户端自行处理*/
						...
						saveCookie(response,"loginName",eLoginName.getTextTrim().substring(0,12));				
